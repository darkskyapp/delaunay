...
(function() {
  ...
  Delaunay = {
    ...
    /* Delaunay.triangulateObjects() is a convenience method that does the
     * tedious work of building a matrix representation of the data for you.
     * Just give it an array of objects and the property used for the position
     * vector within those objects, and it'll return an array of arrays of
     * the passed objects, organized into simplices. Nice, simple,
     * JavaScript-y. */
    triangulateObjects: function(objects, key) {
      var n = Delaunay.dimensions(objects, key),
          m = n + 1,
          matrix = new Array(objects.length * n),
          i, j, position, simplices;

      /* Construct a vertex matrix from the given object list. */
      for(i = 0; i !== objects.length; ++i) {
        position = Delaunay.resolve(objects[i], key);

        for(j = 0; j !== n; ++j)
          matrix[i * n + j] = position[j];
      }

      /* Delaunay triangulate the vertex matrix. */
      matrix = Delaunay.triangulateMatrix(n, matrix);

      /* Convert flat triangle matrix into a nested array of objects. */
      simplices = new Array(matrix.length / m);

      for(i = 0; i !== simplices.length; ++i) {
        simplices[i] = new Array(m);

        for(j = 0; j !== m; ++j)
          simplices[i][j] = objects[matrix[i * m + j]];
      }
